...
```bash
conda install fastapi uvicorn python-dotenv -y
```
...
```bash
pip install fastapi uvicorn python-dotenv -y
```
...
http://127.0.0.1:8000/endpoint_name

replacing the endpoint_name by the name of the desired endpoint

# Default FastAPI endpoints

FastAPI adds the following endpoints by default:
